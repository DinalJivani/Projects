...
NSString *const apiKey = @"AIzaSyAKfDN-uF-RYH1srdkhTGx3ue1rKQvzxo4";
...
-(void)retrieveGooglePlaceInformation:(NSString *)searchWord withCompletion:(void (^)(BOOL isSuccess, NSError *error))completion {
    ...
    if ([self.searchResultsCache objectForKey:searchWord]) {
        ...
        NSString *urlString = [NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/autocomplete/json?input=%@&types=(cities)&language=en&key=%@",searchWord,apiKey];
